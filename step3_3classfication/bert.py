...
def train_model(model, train_loader, dev_loader, num_epochs=2, lr=2e-5, print_every=1,
                save_path='../model/2bert_model.pth', log_path='../log/2bert_log.csv'):
    with open(log_path, 'w', newline='') as csvfile:
        ...
        for epoch in range(num_epochs):
            ...
            with torch.no_grad():
                ...
                precision = precision_score(all_labels, all_predicted, average='macro')  # 使用宏平均
...
# 训练模型并保存
train_model(model, train_loader, dev_loader, num_epochs=10, lr=2e-5, print_every=1, save_path='../model/2bert_model.pth',
            log_path='../log/2bert_log.csv')
